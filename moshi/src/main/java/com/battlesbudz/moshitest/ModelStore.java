...
final class ModelStore {
    ...
    final String marker;
    ModelStore(Context c, boolean codecOnly)throws Exception{
        marker=codecOnly?"verified-codec.json":"verified.json";
        context=c;root=new File(c.getFilesDir(),"models");root.mkdirs();
        try(InputStream in=c.getAssets().open("models.json")){JSONObject all=new JSONObject(new String(Io.bytes(in),java.nio.charset.StandardCharsets.UTF_8)); JSONArray selected=new JSONArray(); for(int i=0;i<all.getJSONArray("files").length();i++){JSONObject item=all.getJSONArray("files").getJSONObject(i);if(!codecOnly||item.getBoolean("codecOnly"))selected.put(item);} manifest=new JSONObject().put("files",selected);}
    }
    boolean ready()throws Exception{
        File verified=new File(root,marker);
        if(!verified.isFile()||!Io.read(verified.toPath()).equals(manifest.toString()))return false;
    ...
    void install(Uri folder,Progress progress)throws Exception{
        ...
        for(int i=0;i<list.length();i++){
            ...
            if(folder==null){
                URL url=new URL("https://huggingface.co/"+item.getString("repository")+"/resolve/"+item.getString("revision")+"/"+path);
        ...
        Io.write(new File(root,marker).toPath(),manifest.toString());
        if(marker.equals("verified.json")) {
            ModelStore codec=new ModelStore(context,true);
            Io.write(new File(root,codec.marker).toPath(),codec.manifest.toString());
        }
