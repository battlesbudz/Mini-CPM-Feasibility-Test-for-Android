...
public final class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle saved){
        ...
        TextView scope=new TextView(this);scope.setText("CPU / Vulkan voice benchmark • Fully local after setup\n\nMoshi / Moshika Q4_K experimental voice engine. Start with the Mimi codec test, then load-only, then full replay. The reference model has a female voice. This does not yet test live conversation or echo cancellation.\n");column.addView(scope);
        ...
        voice=new Spinner(this);voice.setAdapter(new ArrayAdapter<>(this,android.R.layout.simple_spinner_dropdown_item,new String[]{"1. Mimi codec replay · 10 sec","2. Moshi load-only · 750-frame context","3. Moshi voice replay · 20 sec"}));column.addView(voice);
    ...
    private void startRun(){
        try{if(!new ModelStore(this,voice.getSelectedItemPosition()==0).ready())throw new IOException("Install and verify the complete voice pack first.");if(voice.getSelectedItemPosition()!=1&&!new File(getCacheDir(),"input/question.wav").isFile())throw new IOException("Record a question first.");
    ...
    private void refresh(){if(destroyed||status==null)return;boolean running=workerAlive()||SystemClock.elapsedRealtime()<startingUntil;
        if(busy)getWindow().addFlags(android.view.WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);else getWindow().clearFlags(android.view.WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);for(Button b:controls)b.setEnabled(!busy&&!running);duration.setEnabled(!busy&&!running);voice.setEnabled(!busy&&!running);String text=local;
    ...
    private void play(){launch(()->{
        File pcm=new File(getFilesDir(),"last-run/generated.pcm");if(!pcm.isFile()||pcm.length()==0)throw new IOException("No generated speech yet");String metadata=read("report.json");if(metadata.isEmpty())metadata=read("last-run/partial.json");JSONObject report=metadata.isEmpty()?new JSONObject():new JSONObject(metadata);int rate=report.optInt("sampleRate",24000);
